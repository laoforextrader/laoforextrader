...
export default async function ArticleDetailPage({ params }: Props) {
  ...
  const path = `/analysis/${article.slug?.current ?? ""}`
  return (
    <div style={{ background: "#EDEEF2", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <Link href="/analysis"
          ...
          <ArrowLeft size={13} /> ກັບໄປໜ້າ ວິເຄາະ
        </Link>
        ...
        <PostEngagement
          ...
          postUrl={`https://www.laoforextrader.com/analysis/${article.slug?.current ?? ""}`}
        />

        <div style={{ marginTop: 32, paddingTop: 20, borderTop: "1px solid #E5E7EB" }}>
          <Link href="/analysis" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "#2563EB", fontSize: 13, textDecoration: "none", fontWeight: 600 }}>
            <ArrowLeft size={13} /> ກັບໄປໜ້າ ວິເຄາະ
          </Link>
        </div>
      </div>
